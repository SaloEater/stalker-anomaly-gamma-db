...
// Load game data
const index = JSON.parse(fs.readFileSync(path.join(root, "site/data/gamma-0.9.4/index.json"), "utf8"));
const translations = JSON.parse(fs.readFileSync(path.join(root, "site/data/gamma-0.9.4/translations.json"), "utf8"));
const catMap = {};
...
for (const slug of ["rifles", "shotguns", "smgs", "snipers", "launchers", "pistols", "melee"]) {
    const d = JSON.parse(fs.readFileSync(path.join(root, `site/data/gamma-0.9.4/${slug}.json`), "utf8"));
